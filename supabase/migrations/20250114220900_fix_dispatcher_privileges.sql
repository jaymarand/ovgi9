-- Drop any existing is_dispatcher functions
DROP FUNCTION IF EXISTS is_dispatcher(uuid);
DROP FUNCTION IF EXISTS is_dispatcher();
DROP FUNCTION IF EXISTS is_dispatcher(text);

-- Create a function to check if a user is a dispatcher with explicit parameter name and type
CREATE OR REPLACE FUNCTION is_dispatcher(check_user_id uuid)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
GRANT EXECUTE ON FUNCTION is_dispatcher(uuid) TO authenticated;
